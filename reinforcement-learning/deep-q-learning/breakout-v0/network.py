...
class QNetwork:
    def __init__(self, height, width, channel,
        ...
        with tf.variable_scope(name):
            ...
            self.inputs_ = tf.placeholder(tf.float32, [None, height, width, channel], name='inputs')
            ...
            with tf.variable_scope('conv1'):
                ...
                b = tf.get_variable('biases', [16], initializer=tf.constant_initializer(0.0))
                conv = tf.nn.conv2d(self.inputs_, W, strides=[1, 4, 4, 1], padding='VALID')
            ...
            with tf.variable_scope('conv2'):
                ...
                conv = tf.nn.conv2d(self.conv1, W, strides=[1, 2, 2, 1], padding='VALID')
            ...
            #    W = tf.get_variable('weights', [3, 3, 32, 32],
            #                        initializer=tf.truncated_normal_initializer(stddev=5e-2))
            #    b = tf.get_variable('biases', [32], initializer=tf.constant_initializer(0.0))
            #    conv = tf.nn.conv2d(self.conv2, W, strides=[1, 1, 1, 1], padding='VALID')
            #    self.conv3 = tf.nn.relu(tf.nn.bias_add(conv, b), name='conv3')
            ...
            with tf.variable_scope('fc'):
                ...
                reshape = tf.reshape(self.conv2, [-1, reduce(lambda x, y: x * y, s[1:])])
                dim = reshape.get_shape()[1].value
                W = tf.get_variable('weights', [dim, 256],
                                    initializer=tf.truncated_normal_initializer(stddev=4e-2))
                b = tf.get_variable('biases', [256], initializer=tf.constant_initializer(0.1))
